...
def AcertaFase(Fs, f0, f_off, omega, h, f, phasor,num_harm):
    """_summary_
    ...
    """
    f_est = num_harm*f_off # 45 225
    fmais = f0*num_harm + 5

    fmenos = f0*num_harm - 5
    delta_f = -(num_harm*f0 - f_est) ## -5
    ...
    idx1 = np.abs(f - fmenos).argmin() # retorna o indice no array f em que o val é mais procimo de 45 
    idx2 = np.abs(f - fmais).argmin()
    ...
    phi50 = m*(f0*num_harm-f1) + phi1
    phi55 = m*(fmais-f1) + phi1
    ...
    plt.plot(50*num_harm,phi50*180/np.pi,'go')   
    plt.xlabel('Frequência (Hz)')
